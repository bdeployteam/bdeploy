...
[WARNING]
Be aware that the user executing the *master* or *slave* later on *must* be the owner of the root directory. Otherwise *BDeploy* will refuse to start. If you configure *BDeploy* as a service make sure to either run the service as the same user who initialized the root directory, or pass ownership of the root directory to the user running the service before starting it.
