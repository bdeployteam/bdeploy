...
The global capabilities can be changed using the btn:[Global Capabilities] button in the pull-down menu.

image::images/BDeploy_UserAccounts_SetGlobalCapabilities.png[BDeploy User Accounts,align=center,width=480,link="images/BDeploy_UserAccounts_SetGlobalCapabilities.png"]
...
| *READ*
| Only those *Instance Groups* for which a user has *READ* capability are displayed. Users with global *READ* capability can see all *Instance Groups*.
*READ* capability allows the user to read the *Instance Group* without making any modifications.

| *WRITE*
| The *WRITE* capability allows a user to maintain the contents of an *Instance Group* without modifying the *Instance Group* itself. The *WRITE* capability contains the *READ* capability. Users with global *WRITE* capability can maintain all *Instance Groups*.
| *ADMIN*
| The *ADMIN* capability contains the *WRITE* and *READ* capability and allows full access to an *Instance Group*. Users with global *ADMIN* capability have full access to all *Instance Groups* and additionally to the *BDeploy* system configuration.
|===
