...
| Process is *starting*. If the process defined a _startup probe_ (see `<<_app_info_yaml,app-info.yaml>>`), this status is reached once the startup probe indicates a successful startup of the application.
