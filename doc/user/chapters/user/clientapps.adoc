...
|Permissions of the root directory are invalid. Typically happens when just he _'Modify'_ permission is revoked. 
|Make sure to also revoke the _'Write'_ permissions to make the installation read-only or to grant 'Modify' permission.
...
|Make sure that the permissions are set to _'Modify'_ or to _'Read, Execute'_.
