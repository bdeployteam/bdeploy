...
export class SoftwareCardComponent implements OnInit {
  ...
  public diskUsage = '(...)';
  get operatingSystems(): OperatingSystem[] {
    return Array.from(this.softwarePackageGroup.osVersions.keys());
  }
  get versionsCount(): number {
    return Array.from(this.softwarePackageGroup.osVersions.values())
