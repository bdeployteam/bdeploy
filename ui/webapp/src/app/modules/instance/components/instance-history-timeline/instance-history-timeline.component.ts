...
export class InstanceHistoryTimelineComponent implements OnInit {
  ...
  @Output() addToComparison: EventEmitter<any> = new EventEmitter();

  @Input() entries: HistoryEntryDto[] = [];
  @Input() accordionBehaviour = false;
  ...
  closeAll(): void {
    for (const card of this.cards._results) {
      card.close();
    }
  }
