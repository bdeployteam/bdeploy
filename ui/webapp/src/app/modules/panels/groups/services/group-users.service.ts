...
export class GroupUsersService {
  ...
  public updatePermission(user: UserInfo, name: string, modPerm: Permission) {
    const upd: UserPermissionUpdateDto = { user: user.name, permission: modPerm };
    return this.http.post(`${this.apiPath(this.groups.current$.value.name)}/permissions`, [upd]).pipe(measure('Update user permission'));
  }
}
