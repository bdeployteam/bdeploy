...
const colName: BdDataColumn<PortParmGroup> = {
  ...
  data: (r) => r.desc.name,
  tooltip: (r) =>
    `Used in ${r.apps[0].name}${
      r.apps.length > 1
        ? ` and ${r.apps.length - 1} other application${
            r.apps.length > 2 ? 's' : ''
          }`
        : ''
    }`,
...
export class PortsComponent implements OnInit {
  /* template */ columns: BdDataColumn<PortParmGroup>[] = [
    colName,
    colOccurances,
    colPort,
  ];
  ...
  /* template */ exportCsv() {
    ...
    for (const group of this.portEdit.ports$.value.filter(
      (p) => p.desc.type === ParameterType.SERVER_PORT
    )) {
      csv +=
        '\n' +
        group.apps
