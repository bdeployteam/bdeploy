...
const colType: BdDataColumn<AppRow> = {
  id: 'type',
  name: 'Type',
  data: (r) => (!r.template ? 'auto_fix_off' : 'auto_fix_normal'),
  width: '30px',
...
export class AddProcessComponent implements OnInit, OnDestroy {
  private colAdd: BdDataColumn<AppRow> = {
    id: 'add',
    name: 'Add',
    data: (r) => `Add ${!!r.template ? r.template.name : r.app.appDisplayName} to selected node.`,
    icon: (r) => 'add',
  ...
  /* template */ columns: BdDataColumn<AppRow>[] = [colAppName, colType, this.colAdd];
    ...
    vars.subscribe((v) => {
      ...
      if (this.isClientNode(row.node)) {
        ...
        for (const app of row.app.applications) {
          individualCreations.push(this.edit.addProcess(app, row.template, v));
        }
        allCreations = forkJoin(individualCreations);
      } else {
        // only one application may exist with the same ID.
        allCreations = this.edit.addProcess(row.app.applications[0], row.template, v);
      }
