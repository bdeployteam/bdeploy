...
export class NodeDetailsComponent implements OnInit, OnDestroy {
  ...
  /* template */ nodeCfg$ = new BehaviorSubject<InstanceNodeConfigurationDto>(
    null
  );
      ...
      combineLatest([
        ...
        if (!node || !states) {
          ...
          if (cfgs?.nodeConfigDtos?.length) {
            this.nodeCfg$.next(
              cfgs.nodeConfigDtos.find((d) => d.nodeName === node)
            );
          }
          this.nodeVersion = convert2String(
            this.nodeState$.value?.config.version
          );
        }
      })
    );
