...
import { ObjectChangeType, Permission, SoftwareRepositoryConfiguration, UserInfo, UserPermissionUpdateDto } from 'src/app/models/gen.dtos';
...
export class RepositoryUsersService {
  ...
  constructor(private cfg: ConfigService, private http: HttpClient, private changes: ObjectChangesService, private repos: RepositoriesService) {
  ...
  public updatePermission(user: UserInfo, modPerm: Permission) {
    ...
    return this.http.post(`${this.apiPath(this.repo.name)}/permissions`, [upd]).pipe(measure('Update user permission'));
  }
}
