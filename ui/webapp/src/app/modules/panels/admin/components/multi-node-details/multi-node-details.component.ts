...
export class MultiNodeDetailsComponent implements OnInit, OnDestroy {
  ...
  protected downloadingMasterFile$ = new BehaviorSubject<boolean>(false);
  ...
  protected onDelete() {
    ...
    this.dialog
      .confirm(
        `Remove ${nodeName}?`,
        'The multi-node will be removed. <strong>This cannot be undone.</strong>',
        'delete'
      )
      .subscribe((r) => {
        if (r) {
  ...
  protected downloadMasterFile = () => {
    this.downloadingMasterFile$.next(true);
    this.nodeAdmin
      .downloadMultiNodeMasterFile(this.nodeName$.value)
      .pipe(finalize(() => this.downloadingMasterFile$.next(false)))
      .subscribe();
  }
}
